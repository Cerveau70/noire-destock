...
const Marketplace: React.FC<MarketplaceProps> = ({
  ...
  return (
    <div className="max-w-7xl mx-auto w-full py-4 md:py-8 pb-24 md:pb-8 space-y-4 min-w-0 box-border">
      {/* 1. Barre de recherche (marge 16px via page-padding) */}
      <div>
      ...
      {/* 2. Ligne Catalogue + Filtres (alignement 16px bord gauche) */}
      ...
      {/* 3. Pills : style outline aéré et transparent */}
      <div className="filter-bar-scroll scrollbar-hide mb-4 flex flex-row flex-nowrap overflow-x-auto gap-3 px-4 py-2">
        {categories.map((cat) => (
          <button
            key={cat}
            onClick={() => setSelectedCategory(cat)}
            className={`filter-btn shrink-0 whitespace-nowrap px-4 py-2 rounded-lg text-[12px] font-semibold transition-all duration-200
              ${selectedCategory === cat
                ? 'bg-[#064e3b] text-white border border-[#064e3b] shadow-sm'
                : 'bg-transparent border border-gray-200 text-gray-700 hover:border-[#064e3b] hover:text-[#064e3b]'
              }
            `}
          >
            {cat === 'ALL' ? 'Toutes' : cat}
      ...
      {isMobileFiltersOpen && (
        <>
          ...
          <div className="fixed inset-y-0 right-0 w-[85%] max-w-md z-[60] bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
            ...
            <div className="flex-shrink-0 pb-4 px-4 flex justify-between items-center border-b border-gray-100" style={{ paddingTop: 'var(--safe-top)' }}>
            ...
            {/* Sticky Footer — Réinitialiser + Voir les résultats (pb-6 pour barre Android) */}
            <div className="flex-shrink-0 p-4 pt-4 pb-6 border-t border-gray-200 bg-white" style={{ paddingBottom: 'max(24px, calc(16px + env(safe-area-inset-bottom)))' }}>
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        {/* Grille produits : padding 16px (conteneur), gap 12px entre colonnes */}
            ...
            <div className="space-y-6 md:space-y-8">
              {byCategory.map(([category, list]) => (
                <div key={category}>
                  ...
                  <div className="flex flex-row flex-nowrap overflow-x-auto gap-3 pb-4 pl-4 scrollbar-hide snap-x snap-mandatory products-scroll-mobile md:grid md:grid-cols-3 lg:grid-cols-4 md:gap-4 md:overflow-visible md:pb-0 md:pl-0 min-w-0">
                    {list.map((product) => (
