...
const AdminSidebar: React.FC<AdminSidebarProps> = ({ role, onLogout, activeSection, onNavigate }) => {
  ...
  return (
    <>
      ...
      {/* 2. OVERLAY / BACKDROP (Flou derrière le menu quand il est ouvert sur mobile) */}
      {isOpen && (
        <div 
      ...
      <aside
        ...
        w-64 md:w-72 max-w-[85vw] bg-[#0f172a] h-screen flex flex-col text-white font-sans
      ...
      >
        ...
        <div className="p-8 border-b border-gray-800 flex items-center gap-4">
          ...
          <div>
            <h1 className="font-black text-xl leading-none tracking-tighter italic">
              {getBrandLabel().main}<span className="font-light text-emerald-400">{getBrandLabel().sub}</span>
            </h1>
            <span className="text-[10px] text-gray-500 uppercase font-black tracking-[0.2em]">{getRoleLabel()}</span>
          </div>
        </div>
        ...
        <div className="p-6 border-t border-gray-800 bg-[#020617]">
          <div className="flex items-center gap-3 mb-6 px-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-black truncate uppercase tracking-tighter">{getRoleLabel()}</p>
